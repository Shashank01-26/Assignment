...
class EventDetailsWidget extends StatefulWidget {
  final int data;
  const EventDetailsWidget({Key? key, required this.data}) : super(key: key);
...
class _EventDetailsWidgetState extends State<EventDetailsWidget> {
  ...
  Future<Map<String, dynamic>> getEventDetails() async {
    final response = await http.get(
      Uri.parse(
          "https://sde-007.api.assignment.theinternetfolks.works/v1/event/${widget.data}"),
    );
...
class _Content extends StatelessWidget {
  ...
  const _Content({super.key, required this.eventData});
